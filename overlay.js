// ai-overlay.js - Groq FREE Version - Dark Blue
...
(() => {
  ...
  root.innerHTML = `<div id="ai-blue-head"><div><b>AI ASSIST</b> <span>• GROQ FREE • FAST</span></div><div style="display:flex;gap:6px"><button id="ai-b-clear">clear</button><button id="ai-b-close">✕</button></div></div><div id="ai-blue-chat"><div class="ai-b-msg ai-b-bot">✅ Groq Free Mode Active

I can see this page. Fast + free, no OpenAI billing needed.

1. Get free key: console.groq.com/keys
2. Paste below
3. Click Analyze Page

Models: llama-3.3-70b-versatile is best.</div></div><div id="ai-blue-foot"><div id="ai-blue-actions"><button class="ai-b-action" id="ai-b-analyze">↗ Analyze Page</button><button class="ai-b-action" id="ai-b-summarize">◫ Summarize</button><button class="ai-b-action" id="ai-b-extract">⧉ Extract</button></div><div id="ai-blue-settings"><input id="ai-b-key" type="password" placeholder="gsk_... Groq Free Key" /><select id="ai-b-model" style="max-width:180px;background:#070f26;color:#94a3b8;border:1px solid #1e3a8a;border-radius:8px;padding:6px 8px;font-size:10.5px;"><option value="llama-3.3-70b-versatile">llama-3.3-70b-versatile (Best)</option><option value="llama-3.1-8b-instant">llama-3.1-8b-instant (Fastest)</option><option value="llama-3.1-70b-versatile">llama-3.1-70b-versatile</option><option value="mixtral-8x7b-32768">mixtral-8x7b</option><option value="gemma2-9b-it">gemma2-9b-it</option></select></div><div id="ai-blue-input-row"><textarea id="ai-blue-input" rows="2" placeholder="Ask about this page..."></textarea><button id="ai-blue-send">↑</button></div></div>`;
  document.body.appendChild(root);
  const $=id=>document.getElementById(id);
  const chat=$('ai-blue-chat'),input=$('ai-blue-input'),send=$('ai-blue-send'),keyIn=$('ai-b-key'),modelIn=$('ai-b-model');
  keyIn.value=localStorage.getItem('ai_blue_groq_key')||''; modelIn.value=localStorage.getItem('ai_blue_groq_model')||'llama-3.3-70b-versatile';
  keyIn.onchange=()=>localStorage.setItem('ai_blue_groq_key',keyIn.value); modelIn.onchange=()=>localStorage.setItem('ai_blue_groq_model',modelIn.value);
  function getPageContext(){const title=document.title;const url=location.href;let text=document.body.innerText||'';text=text.replace(/\s+/g,' ').trim().slice(0,12000);return{title,url,text};}
  ...
  let history=[{role:'system',content:`You are an AI assistant embedded on a webpage. You see the page content. Be concise, helpful. Date: ${new Date().toDateString()}`}];
  async function askAI(userPrompt,includePage=true){
    if(!keyIn.value) return addMsg('Add your Groq API key first.\n1. Go to console.groq.com/keys\n2. Create free key\n3. Paste here','bot');
    if(includePage){const ctx=getPageContext();userPrompt=`PAGE TITLE: ${ctx.title}\nURL: ${ctx.url}\nCONTENT:\n${ctx.text}\n\nUSER: ${userPrompt}`;}
    addMsg(userPrompt.includes('USER:')?userPrompt.split('USER: ')[1]:userPrompt,'user');
    history.push({role:'user',content:userPrompt});
    const botEl=addMsg('Thinking...','bot'); send.disabled=true;
    try{
      const res=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+keyIn.value},body:JSON.stringify({model:modelIn.value,messages:history,temperature:0.3})});
      const data=await res.json(); if(!res.ok) throw new Error(data.error?.message||JSON.stringify(data));
